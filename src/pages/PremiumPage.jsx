...
// Premium subscription page with Stripe integration.
//
// Three tiers: Free, Premium, Legacy.
// Premium unlocks: AI voice cloning, unlimited memorials, priority support.
// Legacy unlocks: everything + family tree analytics, custom domain, dedicated support.
//
// Route: /premium
...
const PLANS = [
  ...
  },
  {
    ...
    features: [
      ...
      'Custom memorial domain',
      'Dedicated support agent',
      'Early access to new features',
      'Legacy letters (unlimited)',
    ],
    cta: 'Go Legacy',
    highlighted: false,
  },
...
const FEATURES_COMPARE = [
  ...
  { label: 'Priority support', free: '—', premium: '✓', legacy: '✓' },
  { label: 'Dedicated agent', free: '—', premium: '—', legacy: '✓' },
]
      ...
      <div className="px-5 mb-8 text-center">
        <motion.div initial={{ opacity:0, y:20 }} animate={{ opacity:1, y:0 }} transition={{ duration:0.5 }}>
          ...
          <p className="text-sm text-white/45 max-w-md mx-auto leading-relaxed">
            Every memorial is free to create. Premium unlocks deeper ways to remember,
            honour, and connect with those who matter most.
          </p>
        </motion.div>
      </div>

      {/* Plan cards */}
      <div className="px-5 grid grid-cols-1 md:grid-cols-3 gap-4 mb-10">
        {PLANS.map((plan, i) => {
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-white/5">
                ...
                <th className="px-4 py-3 text-[0.6rem] font-bold tracking-widest uppercase text-white/30 text-center">Free</th>
                <th className="px-4 py-3 text-[0.6rem] font-bold tracking-widest uppercase text-gold/60 text-center">Premium</th>
                <th className="px-4 py-3 text-[0.6rem] font-bold tracking-widest uppercase text-white/30 text-center">Legacy</th>
              </tr>
            </thead>
            <tbody>
              {FEATURES_COMPARE.map((row, i) => (
                <tr key={i} className="border-b border-white/5 last:border-0">
                  ...
                  <td className="px-4 py-3 text-xs text-white/40 text-center">{row.free}</td>
                  <td className="px-4 py-3 text-xs text-gold/70 text-center font-medium">{row.premium}</td>
                  <td className="px-4 py-3 text-xs text-white/60 text-center">{row.legacy}</td>
                </tr>
              ))}
            </tbody>
          </table>
